...
export const getImage = async (req, res) => {
    try {
        const response = await prisma.normalmusim.findMany({
            orderBy : {
                createdAt: "desc"
            },
        });
...
export const getImageById = async (req, res) => {
    try {
        const response = await prisma.normalmusim.findUnique({
            where: {
...
export const createImage = async (req, res) => {
    try {
        ...
        const url = `${req.protocol}://${req.get("host")}/public/normal-musim/${fileName}`;
        ...
        await prisma.normalmusim.create({
            data: {
                name: name,
                foto: fileName,
                url: url
...
export const updateImage = async (req, res) => {
    const Image = await prisma.normalmusim.findUnique({
        where: {
            id: Number(req.params.id)
        }
    });
    ...
    if (req.file === null) {
        ...
        const filepath = `./public/normal-musim/${Image.foto}`;
    ...
    try {
        await prisma.normalmusim.update({
...
export const deleteImage = async (req, res) => {
    ...
    try {
        ...
        await prisma.normalmusim.delete({
            where: {
                id : Number (req.params.id)
            }
        });
