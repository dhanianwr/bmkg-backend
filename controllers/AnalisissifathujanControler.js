...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, './public/analisis-sifat-hujan');
    },
...
export const getImage = async (req, res) => {
    try {
        const response = await prisma.analisissifathujan.findMany({
            orderBy : {
                createdAt: "desc"
            },
        });
...
export const getImageById = async (req, res) => {
    try {
        const response = await prisma.analisissifathujan.findUnique({
            where: {
...
export const createImage = async (req, res) => {
    try {
        ...
        const url = `${req.protocol}://${req.get("host")}/public/analisis-sifat-hujan/${fileName}`;
        ...
        await prisma.analisissifathujan.create({
            data: {
                name: name,
                foto: fileName,
                url: url
...
export const updateImage = async (req, res) => {
    const Image = await prisma.analisissifathujan.findUnique({
        where: {
            id: Number(req.params.id)
        }
    });
    ...
    if (req.file === null) {
        ...
        const filepath = `./public/analisis-sifat-hujan/${Image.foto}`;
    ...
    try {
        await prisma.analisissifathujan.update({
...
export const deleteImage = async (req, res) => {
    ...
    try {
        ...
        await prisma.analisissifathujan.delete({
            where: {
                id : Number (req.params.id)
            }
        });
