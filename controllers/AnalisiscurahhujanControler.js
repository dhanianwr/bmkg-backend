...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, './public/analisis-curah-hujan');
    },
...
export const getImage = async (req, res) => {
    try {
        const response = await prisma.analisiscurahhujan.findMany({
            orderBy: {
                createdAt: "desc",
              },
        });
...
export const getImageById = async (req, res) => {
    try {
        const response = await prisma.analisiscurahhujan.findUnique({
            where: {
...
export const createImage = async (req, res) => {
    try {
        ...
        const url = `${req.protocol}://${req.get("host")}/public/analisis-curah-hujan/${fileName}`;
        ...
        await prisma.analisiscurahhujan.create({
            data: {
                name: name,
                foto: fileName,
                url: url
...
export const updateImage = async (req, res) => {
    const Image = await prisma.analisiscurahhujan.findUnique({
        where: {
            id: Number(req.params.id)
        }
    });
    ...
    if (req.file === null) {
        ...
        const filepath = `./public/analisis-curah-hujan/${Image.foto}`;
    ...
    try {
        await prisma.analisiscurahhujan.update({
...
export const deleteImage = async (req, res) => {
    ...
    try {
        ...
        await prisma.analisiscurahhujan.delete({
            where: {
                id : Number (req.params.id)
            }
        });
